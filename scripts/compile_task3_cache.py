...
for compiler_tag in all_the_compilers.keys():
    for flag in defs.optimization_flags.keys():
        os.makedirs(
            os.path.join(
                defs.vec_cache_build_dir, compiler_tag, flag, "asm", "dgesv_rs"
            ),
            exist_ok=True,
        )
...
c_files = []
include_dirs = set()
for dirpath, dirnames, filenames in os.walk(defs.src_vec_cache_dir):
...
for compiler_tag, compiler_name in all_the_compilers.items():
    ...
    for o_tag, o_flag in optimization.items():
        output_dir = os.path.join(defs.vec_cache_build_dir, compiler_tag, o_tag)
